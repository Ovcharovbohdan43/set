...
import { createBudget, deleteBudget, fetchBudget, fetchBudgets, recordSnapshot, updateBudget } from './api';
const budgetsKey = ['budgets'];
const budgetKey = (id) => ['budgets', id];
...
export function useRecordSnapshotMutation() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ budgetId, actualCents, projectedCents, snapshotDate }) => recordSnapshot(budgetId, actualCents, projectedCents, snapshotDate),
        onSuccess: async (data) => {
            await queryClient.invalidateQueries({ queryKey: budgetsKey });
