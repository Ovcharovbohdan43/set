...
## 3. Executed Test Commands (2025-11-21)
...
## 10. Beta Release Validation (Stage 10)
- **Scope**: Hardening/telemetry/beta checks (regression suite, sync/load tests, updater readiness, security review).
- **What to run**:
  - Full regression: `pnpm lint`, `pnpm typecheck`, `pnpm test`, `cargo test`, `pnpm test:e2e`.
  - Performance/load: simulate reminders/sync over 30 days (script TBD), monitor memory <150 MB idle.
  - Security: `pnpm audit`, `cargo audit`, threat-model checklist (JWT/crypto/logging).
  - Packaging: `pnpm tauri build --target x86_64-pc-windows-msvc` (MSI), signer secrets in CI.
  - Updater: pending schema/tooling support—use manual installer for beta.

## 11. Operations & Monitoring (Stage 11)
- **Scope**: Post-beta operations, monitoring, and support runbooks.
- **Automated**: CI scheduled `nightly-sync` job (see `.github/workflows/ci.yml`) plus planned Playwright smoke on cron.
